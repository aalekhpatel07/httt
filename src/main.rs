...
async fn handle_connection(mut stream: TcpStream, peer: SocketAddr, duration: Duration) {
    ...
    println!("[ETA {:.3}s] Writing {} bytes to peer {} at the rate of 1 byte per {:#?}", duration.mul_f32(response.len() as f32).as_secs_f32(), response.len(), peer, duration);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let args = Args::parse();
    let listener = TcpListener::bind(format!("0.0.0.0:{}", args.port).parse::<SocketAddr>()?).await?;

    while let Ok((stream, peer)) = listener.accept().await {
        tokio::task::spawn(handle_connection(stream, peer, Duration::from_secs_f64(args.interval)));
    }
